...
namespace R8
{
    public partial class FormMain : Form
    {
        ...
        private void FormMain_Load(object sender, EventArgs e)
        {
            ...
            //20180124 add : R8 要讀 Mac
            StringBuilder macSB = new StringBuilder(R7.STRING_SIZE);
            R7.GetMacAddress(macSB, R7.STRING_SIZE);
            R8.MacAddress = macSB.ToString();
            macSB.Clear();
            ...
            //20180115 leo: 依信件指示增加 login：
            /*
             按下 Login Button 之後，透過 curl post 到： http://www.openrobot.club/api/OpenR8/GetCommunityEditionLicense
             取得 license json 存到 R7.license 檔案。
             */
            //估計以後連 R7 也要 Login ，因此呼叫 curl 的 function 包到 R7 裡面。
            //R7_API int R7_Login(char *buffer, int bufferLength, char *userName, char *password)
            /*
            if (true)
            {
                string r7LicenseFilePath = "r7.license";
                if (File.Exists(r7LicenseFilePath))
                {
                    string licenseText = File.ReadAllText(r7LicenseFilePath);
                    //當 license 編碼格式確定後，這邊要加
                    //依據 MacAddress 與 licenseText 解碼，然後依解碼內容設置 year 、 month 、 day
                    //MessageBox.Show("licenseText = " + licenseText);
                    
                    year = 0;
                    month = 0;
                    day = 0;
                    StringBuilder licenseTextStringBuilder = new StringBuilder(licenseText);

                    R7.CheckLicense(licenseTextStringBuilder, ref year, ref month, ref day);
                    //{{"result":"OK","email":"leo@leaderg.com","mac":"112233445566","licenseKey":"db38d303f4b7893192f7b127e51e4f7e5c457dec28937199184d4ecc0392f6c2","licenseDate":"2018-01-23"}}
                    if (new DateTime(year, month, day) < DateTime.Now)
                    {
                       // MessageBox.Show("License out date (" + year + "-" + month + "-" + day + ")");
                ...
                {
                    //20180124 白名單...不管 r7.license 有沒有，都進去  R7.CheckLicense
                    year = 0;
                    month = 0;
                    day = 0;
                    R7.CheckLicense(null, ref year, ref month, ref day);
            ...
            */
            ...
            if (new DateTime(year, month, day) < DateTime.Now)
            {
                ...
                MessageBox.Show("OpenR8 community edition (" + version + ") is expired.");
                this.Dispose();
                this.Close();
                Environment.Exit(0);
                Application.Exit(); //這邊如果只關掉 Form 會當機，要用 Application.Exit 或 Environment.Exit.
            }
            */
            // License --
